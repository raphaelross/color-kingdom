...
void main() {
  ...
  test(
    'logical region map is preserved while visual fill map expands ownership',
    () async {
      ...
      final logicalBytes = await logicalImage.toByteData(
      ...
      final fillBytes = await fillImage.toByteData(
        format: ui.ImageByteFormat.rawRgba,
      );
      ...
      expect(
        ...
        isNotNull,
        reason:
            'Expected at least one pixel claimed by visual fill map but unowned in logical map',
      );
      ...
      expect(
        logicalRegionAtSample,
        isNull,
        reason:
            'Sample halo pixel must remain unassigned in logical map for hit testing',
      );
      ...
      for (var i = 0; i < metadata.imageWidth * metadata.imageHeight; i++) {
        ...
        expect(
          ...
          logicalRegion,
          reason:
              'Visual fill map must not overwrite logical interior ownership',
        );
      }
  ...
  test('expansion metrics remain bounded and deterministic for 0..4px', () async {
    ...
    final metadataJson =
        jsonDecode(await rootBundle.loadString(metadata.metadataAssetPath!))
            as Map<String, dynamic>;
    final visualFill = metadataJson['visualFillMap'] as Map<String, dynamic>;
    final metricsByExpansion =
        visualFill['metricsByExpansion'] as Map<String, dynamic>;
    ...
    expect(
      ...
      198,
      reason:
          'The promoted 175-region Lovely Kitten fill map has a new canonical '
          '1px ambiguous-pixel count while still preserving bounded expansion '
          'and zero bleed risk.',
    );
    ...
    expect(
      addedRatio,
      lessThan(0.07),
      reason: 'Expansion should stay a narrow boundary band',
    );
  });

  test(
    'composited representative halo pixel is no longer white after visual fill',
    () async {
      final page = sampleLovelyKittenPage;
      final metadata = page.rasterRegionMetadata!;
      ...
      final foregroundImage = await _decodeImage(
        foregroundAsset.buffer.asUint8List(),
      );
      ...
      for (var y = 0; y < metadata.imageHeight && sampleX == null; y++) {
        for (var x = 0; x < metadata.imageWidth; x++) {
          ...
          final foregroundAlpha = foregroundBytes!.buffer
              .asUint8List()[offset + 3];
      ...
      final logicalPixels =
          RasterRegionColoringRenderer.buildRegionPixelsForTesting(
      ...
      final logicalBuffer = Uint8List(
        metadata.imageWidth * metadata.imageHeight * 4,
      );
      final fillBuffer = Uint8List(
        metadata.imageWidth * metadata.imageHeight * 4,
      );
      ...
      final foregroundAtSample =
          RasterRegionColoringRenderer.sampleRgbaAtOffset(
      ...
      expect(
        ...
        0,
        reason:
            'Baseline logical fill should not own representative halo sample pixel',
      );
      expect(
        expandedUnder[3],
        255,
      ...
      expect(
        expandedComposite[0] > expandedComposite[1],
        isTrue,
        reason:
            'Expanded composite should carry selected red fill tint at halo sample',
      );
      expect(
        expandedComposite[1] < baselineComposite[1],
        isTrue,
        reason:
            'Expanded composite should reduce gray/white halo channel intensity',
      );
