...
from suitability_classifier import ProfileThresholds, SuitabilityConfig, classify_segmentation_result
...
def test_tiny_region_retained_in_master_excluded_for_children() -> None:
    ...
    _box(img, 95, 10, 10, 10, thickness=1)
...
def test_thin_sliver_region_can_be_rejected() -> None:
    img = _blank(180, 120)
    _box(img, 10, 10, 60, 70, thickness=2)
    _box(img, 100, 10, 70, 5, thickness=1)
    ...
    sliver = min(seg.metadata["acceptedRegions"], key=lambda r: r["features"]["aspectRatio"])
    assert sliver["profiles"]["childrenDetailed"]["included"] is False
    assert "TOO_THIN" in sliver["profiles"]["childrenDetailed"]["reasons"]
...
def test_profile_configuration_changes_results_predictably() -> None:
    ...
    strict = SuitabilityConfig(
        children_detailed=ProfileThresholds(
            ...
            min_bounding_dimension_percent=10.0,
